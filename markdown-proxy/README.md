...
Automatically syncs fetched content to your Obsidian vault using the [obsidian-cli](https://github.com/kepano/obsidian-skills) skill.
- Preserves YAML frontmatter for better indexing
- Supports custom folder targeting (e.g., `Inbox/` or `Clippings/`)
- Silent execution to keep your workflow focused
...
**新增：Obsidian 同步支持** 🚀
配合 [obsidian-cli](https://github.com/kepano/obsidian-skills) skill，自动将抓取的文章同步到 Obsidian 库。
- 完美支持 YAML frontmatter，便于 Obsidian 库管理
- 支持自定义保存目录（如 `Inbox/` 或 `Articles/`）
- 静默后台执行，不干扰当前工作流
