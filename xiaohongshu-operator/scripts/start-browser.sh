#!/bin/bash
#
# Xiaohongshu Operator — Start BrowserWing Service
#
...
#   bash <skill-dir>/scripts/start-browser.sh --port 9222  # specify port
...
# Configuration
XHS_DIR="$HOME/.xiaohongshu-operator"
CONFIG_FILE="$XHS_DIR/config.toml"
LOG_DIR="$XHS_DIR/log"
DATA_DIR="$XHS_DIR/data"
PORT=9222
HOST="127.0.0.1"
...
# First, check if BrowserWing is already running on this port
if curl -s "http://$HOST:$PORT/api/v1/executor/help" > /dev/null 2>&1; then
  echo "✅ BrowserWing service is already running on port $PORT"
  echo "   (This may have been started by xiaohongshu-operator or x-com-operator)"
  exit 0
fi

# Let's dynamically check if any browserwing process is running on another port, if user didn't specify one
if [ "$PORT" = "9222" ]; then
  ...
  if [ -n "$EXISTING_PORT" ] && [ "$EXISTING_PORT" != "$PORT" ]; then
    if curl -s "http://$HOST:$EXISTING_PORT/api/v1/executor/help" > /dev/null 2>&1; then
      echo "✅ Found existing BrowserWing service running on port $EXISTING_PORT"
      echo "   Use that port for interactions instead."
      PORT=$EXISTING_PORT
      exit 0
    fi
  fi
...
# Start BrowserWing
echo "🚀 Starting Xiaohongshu Operator BrowserWing on port $PORT..."
...
for i in {1..10}; do
  ...
  if curl -s "http://$HOST:$PORT/api/v1/executor/help" > /dev/null 2>&1; then
    ...
    echo ""
    echo "📌 Make sure you've logged in to xiaohongshu.com in $BROWSER_NAME beforehand."
    echo ""
