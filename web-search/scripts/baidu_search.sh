#!/bin/bash
# 百度千帆智能搜索 API (普通版/非高性能版) — 每天 1000 次免费
...
if [ -z "$JSON_INPUT" ]; then
    ...
    echo ""
    echo "百度千帆智能搜索 API (普通版)"
    echo "文档: references/baidu_slow_api.md"
    echo ""
    echo "Required:"
    ...
    echo "  model: string - 模型名称 (default: ernie-4.5-turbo-32k)"
    echo "  recency: string - 时间筛选 (week, month, semiyear, year)"
    echo "  include_domains: array<string> - 指定站点搜索 (e.g., [\"zhihu.com\"])"
    echo ""
    echo "Environment:"
    echo "  BAIDU_SEARCH_API_KEY - Required. 百度千帆 API Key"
    echo ""
    echo "Example:"
    echo "  ./baidu_search.sh '{\"query\": \"人工智能最新进展\", \"recency\": \"week\"}'"
    exit 1
fi

if [ -z "$BAIDU_SEARCH_API_KEY" ]; then
    echo "Error: BAIDU_SEARCH_API_KEY environment variable is required"
    echo "获取方式: https://console.bce.baidu.com/qianfan/studio/resource"
    exit 1
fi
...
MODEL=$(echo "$JSON_INPUT" | jq -r '.model // "ernie-4.5-turbo-32k"')
...
# Build request body for /v2/ai_search/chat/completions
REQUEST_BODY=$(jq -n \
    --arg query "$QUERY" \
    --arg model "$MODEL" \
...
# Call Baidu Qianfan AI Search API (Slow Version)
