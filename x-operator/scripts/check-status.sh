#!/bin/bash
#
# Check the status of the X.com Operator BrowserWing service
#
...
echo "🔍 Checking X.com Operator status..."
...
if echo "$PAGE_URL" | grep -q "login\|Login\|sign-in\|i/flow"; then
  echo "⚠️  Browser appears to be on a login page"
  echo "   Please open Brave manually and log in to x.com first"
else
...
echo "   Config: ~/.x-com-operator/config.toml"
echo "   Logs:   ~/.x-com-operator/log/"
